...
$plugins->add_hook("misc_start", "designconfigurator_misc");
...
function designconfigurator_usercp() {
	...
	if (!$db->table_exists("designs_users")) {
		if($mybb->usergroup['cancp'] == 1) {
			error("Das Plugin 'Design Konfigurator' muss geupdatet werden! Rufe dafür BOARDLINK/misc.php?action=designconfigurator_update auf.");	
			return;
		} else {
			error("Das Plugin 'Design Konfigurator' muss geupdatet werden! Sag dem Team bescheid dafür.");	
			return;
		}
	} else {
		// DESIGNNAME
...
// Update Funktion
function designconfigurator_misc() {

    global $db, $cache, $mybb, $lang, $templates, $theme, $header, $headerinclude, $footer, $page;

	if($mybb->get_input('action') == "designconfigurator_update"){
		...
		if ($db->table_exists("designs_users")) {
			echo "Das Plugin ist schon geupdatet worden auf die aktuellste Version.";
		} else {
			// DATENBANK ERSTELLEN
			$db->query("CREATE TABLE ".TABLE_PREFIX."designs_users(	
				`duid` int(10) unsigned NOT NULL AUTO_INCREMENT,
